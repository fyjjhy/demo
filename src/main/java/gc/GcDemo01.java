...
public class GcDemo01 {
    ...
    public static GcDemo01 SAVE_HOOK = null;

    public static void main(String[] args) throws InterruptedException {
        // 新建对象，因为SAVE_HOOK指向这个对象，对象此时的状态是(unfinalized，reachable)
        SAVE_HOOK = new GcDemo01();
        // 此时，刚才创建的对象就不可达了，因为没有句柄指向它了，对象此时的状态是(unreachable，unfinalized)
        SAVE_HOOK = null;
        // 强制系统执行垃圾回收，系统发现刚才创建的对象处于unreachable状态，并检测到这个对象的类覆盖了finalize方法，
        // 因此把这个对象放入F-Queue队列，由低优先级线程执行它的finalize方法，
        ...
        // 但接下来在finalize方法中又执行了SAVE_HOOK = this;这句话，又有句柄指向这个对象了，
        // 对象又可达了。因此对象的状态又变成了(reachable, finalized)状态。
        Thread.sleep(500);
        // 这里对象处于(reachable,finalized)状态应该是合理的。对象的finalize方法被执行了，
